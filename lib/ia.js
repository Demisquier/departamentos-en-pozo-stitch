...
// Detecta el barrio del catálogo mencionado en la query (el nombre real más largo que
// aparezca). Sin listas hardcodeadas: usa los barrios que existen en el catálogo.
export function detectBarrio(items, q) {
  const t = norm(q);
  let best = "";
  for (const p of items || []) {
    const b = (p.barrio || "").trim();
    if (!b) continue;
    const nb = norm(b);
    if (nb && nb.length >= 4 && t.includes(nb) && nb.length > best.length) best = b;
  }
  return best;
}

// Estadísticas de mercado de un barrio: cantidad + precio/m² y precio "desde" (mediana).
// Alimenta el "analista de barrio" del chat para responder con datos reales (confianza).
export function statsBarrio(items, barrio) {
  if (!barrio) return null;
  const nb = norm(barrio);
  const sel = (items || []).filter((p) => norm(p.barrio).includes(nb));
  ...
  return { barrio, n: sel.length, m2Med: med(sel.map((p) => p.precioM2)), desdeMed: med(sel.map((p) => p.precioDesde)) };
}
...
function scored(items, q) {
  ...
  const ambMatch = t.match(/(\d)\s*(amb|dorm|ambiente|dormitorio)/) || (/mono ?ambiente|monoamb/.test(t) ? [null, "1"] : null);
