...
// Mapea los desarrollos crudos (WP/JSON) al shape que consume CatalogoFiltros/ProjectCard.
// Reutilizado por el catálogo general y por las landings por barrio.
export function mapDesarrollos(items) {
  const mapped = (items || []).map((node) => {
    ...
    const desarrolladora = expandComercializa(acfAny(node, ['desarrolladora', 'constructora']) || '');
...
// Barrio "cabecera" (colapsa Palermo Hollywood/Soho/Botánico -> Palermo) para agrupar mejor.
function topBarrio(b) {
  return String(b || '').startsWith('Palermo') ? 'Palermo' : String(b || '');
}

// Proyectos SIMILARES para el carrusel de la ficha. Puntúa por cercanía real:
//  mismo barrio exacto (60) > mismo barrio cabecera/Palermo (45) > proximidad de precio
//  (hasta 25) > misma etapa (15) > desempate: tener foto (8) y precio (4).
// `mapped` es el catálogo ya mapeado (mapDesarrollos). Devuelve hasta `limit` items.
export function similaresDesarrollos(currentSlug, mapped, opts = {}, limit = 10) {
  const { barrio, precioDesde, precioM2, etapa } = opts;
  const curTop = topBarrio(barrio);
  const scored = (mapped || [])
    .filter((m) => m.slug && m.slug !== currentSlug)
    .map((m) => {
      let s = 0;
      if (m.barrio && barrio && m.barrio === barrio) s += 60;
      else if (topBarrio(m.barrio) && curTop && topBarrio(m.barrio) === curTop) s += 45;
      if (m.etapa && etapa && m.etapa === etapa) s += 15;
      // Proximidad de precio: preferimos precio total (precioDesde); si no, precio/m².
      if (precioDesde && m.precioDesde) {
        const diff = Math.abs(precioDesde - m.precioDesde) / precioDesde;
        if (diff <= 0.5) s += Math.round(25 * (1 - diff / 0.5));
      } else if (precioM2 && m.precioM2) {
        const diff = Math.abs(precioM2 - m.precioM2) / precioM2;
        if (diff <= 0.5) s += Math.round(20 * (1 - diff / 0.5));
      }
      if (m.imagen) s += 8;                              // cards con foto se ven mejor
      if (m.precioDesde || m.precioM2) s += 4;           // y con precio
      return { m, s };
    });
