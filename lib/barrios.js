...
export const NEARBY_CATALOGO = {
  "palermo":       ["colegiales", "villa-urquiza", "belgrano"],
  "caballito":     ["almagro", "palermo"],
  "puerto-madero": ["san-telmo", "belgrano"],
  "belgrano":      ["nunez", "colegiales", "coghlan", "villa-urquiza"],
  "nunez":         ["belgrano", "coghlan", "saavedra"],
  "villa-urquiza": ["coghlan", "colegiales", "belgrano"],
  "colegiales":    ["belgrano", "palermo", "coghlan", "villa-urquiza"],
  "coghlan":       ["nunez", "belgrano", "villa-urquiza", "saavedra"],
  "saavedra":      ["nunez", "coghlan"],
  "almagro":       ["caballito", "palermo"],
  "san-telmo":     ["puerto-madero"],
};

// ¿El barrio granular de un proyecto pertenece a la landing `slug`? (prefijo, case-insensitive)
export function matchBarrioCatalogo(barrioProyecto, slug) {
  const conf = BARRIO_CATALOGO[slug];
  if (!conf) return false;
  const b = String(barrioProyecto || "").toLowerCase();
  return conf.match.some((p) => b.startsWith(p.toLowerCase()));
}
