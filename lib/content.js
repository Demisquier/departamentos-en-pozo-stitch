...
function loadData(name) {
  ...
  if (name === "posts") {
    ...
    try {
      ...
      // Override de imagen destacada por slug desde data/post-images.json
      // (fotos con licencia + credito; reemplaza destacadas faltantes o mal recortadas).
      try {
        const _pimg = JSON.parse(
          fs.readFileSync(path.join(process.cwd(), "data", "post-images.json"), "utf8")
        );
        if (Array.isArray(val)) {
          for (const _p of val) {
            const _ov = _pimg[_p && _p.slug];
            if (_ov && _ov.url) {
              _p._embedded = _p._embedded || {};
              _p._embedded["wp:featuredmedia"] = [{ source_url: _ov.url }];
              _p.meta = Object.assign({}, _p.meta || {}, {
                image_credit: _ov.credit || "",
                image_credit_url: _ov.credit_url || "",
              });
            }
          }
        }
