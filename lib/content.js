...
import { generarDescripcion } from "./descripcion-auto";
...
function loadData(name) {
  ...
  if (name === "desarrollo") {
    try {
      ...
      const dx4 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-4.json"), "utf8"));
      if (Array.isArray(dx4) && dx4.length) val = [...(Array.isArray(val) ? val : []), ...dx4];
      const dx5 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-5.json"), "utf8"));
      if (Array.isArray(dx5) && dx5.length) val = [...(Array.isArray(val) ? val : []), ...dx5];
      const dx6 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-6.json"), "utf8"));
      if (Array.isArray(dx6) && dx6.length) val = [...(Array.isArray(val) ? val : []), ...dx6];
      const dx7 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-7.json"), "utf8"));
      if (Array.isArray(dx7) && dx7.length) val = [...(Array.isArray(val) ? val : []), ...dx7];
      const dx8 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-8.json"), "utf8"));
      if (Array.isArray(dx8) && dx8.length) val = [...(Array.isArray(val) ? val : []), ...dx8];
      const dx9 = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "desarrollo-extra-9.json"), "utf8"));
      if (Array.isArray(dx9) && dx9.length) val = [...(Array.isArray(val) ? val : []), ...dx9];
    } catch {}
    // EXCLUIR portales/agregadores (Mudafy es un portal, no una desarrolladora): sus fichas
    // son re-publicaciones y no deben rutear leads ni figurar en el catalogo.
    if (Array.isArray(val)) {
      val = val.filter(function (n) {
        if (n.slug === "constituyentes-3309-agronomia" || n.slug === "unico-cordoba-recoleta") return false;
        var s = (((n.acf && n.acf.comercializadora) || "") + " " + ((n.acf && n.acf.desarrolladora) || "")).toLowerCase();
        return s.indexOf("mudafy") === -1;
      });
    }
    ...
    // Descripción auto para fichas con contenido pobre (<200 chars), generada de los
    // datos del propio nodo (no pisa contenido real). Corre tras aplicar overrides.
    try {
      if (Array.isArray(val)) {
        for (const item of val) {
          const cur = ((item.content && item.content.rendered) || "").replace(/<[^>]+>/g, "").trim();
          if (cur.length < 200) {
            const _g = generarDescripcion(item);
            if (_g) item.content = { ...(item.content || {}), rendered: _g };
          }
        }
      }
    } catch {}
  ...
  if (name === "desarrolladora") {
    ...
    // Overrides de inmobiliaria: proyectos que comercializa (inm_proyectos_slug) por id,
  // sin tocar inmobiliaria.json. Habilita la landing /inmobiliaria/{slug}/ y su seccion.
  if (name === "inmobiliaria") {
    try {
      const ov = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data", "inmobiliaria-overrides.json"), "utf8"));
      if (ov && typeof ov === "object" && Array.isArray(val)) {
        for (const item of val) {
          const o = ov[item.id];
          if (!o) continue;
          item.meta = item.meta || {};
          if (o.inm_proyectos_slug && !item.meta.inm_proyectos_slug) item.meta.inm_proyectos_slug = o.inm_proyectos_slug;
        }
      }
    } catch {}
  }
...
function mapInmo(d) {
  ...
  return {
    ...
    proyectosSlug: (m.inm_proyectos_slug || "").split(",").map((s) => s.trim()).filter(Boolean),
    landeable: (m.inm_proyectos_slug || "").split(",").map((s) => s.trim()).filter(Boolean).length > 0,
  };
}
...
// Una inmobiliaria por slug (para su landing) + los proyectos en pozo que comercializa.
// Los proyectos salen SOLO de la curaduria explicita (meta.inm_proyectos_slug); no hay
// auto-match por nombre (las inmobiliarias no firman la ficha del proyecto).
export async function getInmobiliariaBySlug(slug) {
  const all = await getInmobiliarias();
  let inmo = all.find((d) => d.slug === slug) || null;
  if (!inmo) { const _ex = await getInmobiliariasExtra(); inmo = (_ex || []).find((d) => d.slug === slug) || null; }
  if (!inmo) return null;
  const allP = await getDesarrollos();
  const byId = new Map((allP || []).map((p) => [p.slug, p]));
  const proyectos = (inmo.proyectosSlug || []).map((s) => byId.get(s)).filter(Boolean);
  return { inmo, proyectos };
}

// Comercializadoras fuera del directorio curado (sinteticas): 1 landing c/u por sus proyectos.
export async function getInmobiliariasExtra() {
  const local = loadData("inmobiliaria-extra");
  if (!hasLocal(local)) return [];
  return local.map((d) => ({ ...mapInmo(d), sintetica: true }));
}
