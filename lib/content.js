...
export function featuredImage(node) {
  ...
  if (rel && /^https?:\/\//i.test(rel)) {
    return `https://images.weserv.nl/?url=${encodeURIComponent(rel.replace(/^https?:\/\//, ""))}&w=1200&output=jpg`;
  }
  return rel;
}
...
export function proxyImage(url, w = 1600) {
  ...
  if (!r) return null;
  if (/^https?:\/\//i.test(r)) {
    return `https://images.weserv.nl/?url=${encodeURIComponent(r.replace(/^https?:\/\//, ""))}&w=${w}&output=jpg`;
  }
  return r;
}
