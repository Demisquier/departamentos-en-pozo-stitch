...
// ---- Fallback a WordPress (sólo si falta el dato local) ----
const WP = (process.env.WP_HOST ? `https://${process.env.WP_HOST}` : "https://cms.departamentosenpozo.com.ar") + "/wp-json/wp/v2";
async function wpGet(pathq) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), 12000);
  try {
    const res = await fetch(`${WP}${pathq}`, { next: { revalidate: 300 }, signal: ctrl.signal });
...
export async function getRankMathSchema(pubPath) {
  ...
  if (schemas && typeof schemas === "object") {
    // El archivo local existe (aunque no tenga esta ruta): NO llamamos a WP.
    const s = schemas[pubPath || "/"];
    return Array.isArray(s) ? s : [];
  }
  // fallback WP (sólo si el archivo local de schemas no existe todavía)
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), 5000);
  try {
    const publicUrl = `https://departamentosenpozo.com.ar${pubPath || "/"}`;
    const endpoint = `https://${WP_HOST}/wp-json/pozo/v1/head?url=${encodeURIComponent(publicUrl)}`;
    const res = await fetch(endpoint, { next: { revalidate: 300 }, signal: ctrl.signal });
    if (!res.ok) return [];
