...
// Directorio de desarrolladoras (CPT `desarrolladora`). Trae TODAS paginando de a 100.
// Devuelve objetos normalizados {nombre, web, desc, destacada}. Server-side → el
// listado sale en el HTML estático (SEO safe), el buscador es enhancement client.
function mapDev(d) {
  const m = d.meta || {};
...
// Directorio de inmobiliarias (CPT `inmobiliaria`). Mismo patrón que desarrolladoras,
// sin proyectos. Server-side → listado en el HTML estático (SEO).
...
export async function getDesarrolladoraBySlug(slug) {
  const a = await wpGet(`/desarrolladora?slug=${slug}&_fields=id,slug,title,meta`);
  const dev = Array.isArray(a) && a[0] ? mapDev(a[0]) : null;
  if (!dev) return null;
  let proyectos = [];
  if (dev.proyectosSlug.length) {
    const all = await getDesarrollos();
    proyectos = (all || []).filter((p) => dev.proyectosSlug.includes(p.slug));
  }
