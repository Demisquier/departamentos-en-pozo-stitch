...
export async function getDesarrolladoras() {
  ...
  for (let page = 1; page <= 5; page++) {
    const batch = await wpGet(`/desarrolladora?per_page=100&page=${page}&_fields=id,title,meta`);
    if (!Array.isArray(batch) || batch.length === 0) break;
    for (const d of batch) {
      out.push({
        id: d.id,
        nombre: (d.title && (d.title.rendered || d.title.raw)) || "",
        web: (d.meta && d.meta.dev_web) || "",
        desc: (d.meta && d.meta.dev_desc) || "",
        destacada: !!(d.meta && d.meta.dev_destacada),
      });
    }
    if (batch.length < 100) break;
  }
  // Destacadas primero, después alfabético.
